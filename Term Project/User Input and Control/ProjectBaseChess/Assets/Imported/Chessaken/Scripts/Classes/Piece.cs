...
*/
	public class Piece : MonoBehaviour
	{
		...
		// List of moves the piece can move, starting from its current position
		private MoveType move_type;
		// Type of move the piece is going to move
		private Piece castling_tower;
		// Once we know which tower the king is trying to castle, we save it here

		public List<Coordinate> break_points = new List<Coordinate> ();
		// Coordinates that will break the piece's direction
		public bool started;
		// StartOnly MoveType controller, set to true when the piece moves for the first time
    ...
    */
		...
		private bool checkCanMove (Square square)
		{
			...
			if (square.holding_piece == null && checkBreakPoint (coor_move) && checkValidCheckKingMove (square))
				return true;
			return false;
		}
		...
		private bool checkCanEat (Square square)
		{
			...
			if (square.holding_piece != null && square.holding_piece.team != team && checkBreakPoint (coor_move) && checkValidCheckKingMove (square))
				return true;
			return false;
		}
		...
		{
			if (checkCanEat (square) || checkCanMove (square))
				return true; 
			return false;
		}
